...
class Arm3Link:
    ...
    def __init__(self, q=None, q0=None, L=None):
        ...
        self.q = [.3, .3] if q is None else q
        # some default arm positions
        self.q0 = np.array([np.pi/4, np.pi/4]) if q0 is None else q0
        # arm segment lengths
        self.L = np.array([1, 1]) if L is None else L

        self.max_angles = [np.pi, np.pi]
        self.min_angles = [0, 0]
